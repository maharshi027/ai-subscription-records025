  ...
  const loadData = useCallback(async () => {
    try {
      const [sumRes, txnRes, subRes] = await Promise.all([
        axios.get(
          `/api/transactions/summary/${now.getFullYear()}/${now.getMonth() + 1}`,
        ),
      ...
      setInsights([
        {
          type: "tip",
          title: "Keep going",
          message: "Add transactions to unlock AI insights.",
    ...
    <div>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div
            style={{
              fontSize: 22,
              fontWeight: 700,
              color: savRate >= 20 ? "#34d399" : "#f87171",
            }}
          >
            {savRate}%
          </div>
      ...
      {subAlert && (
        <div className="zombie-alert" style={{ marginBottom: 20 }}>
          ⚠️ <strong>Subscription Graveyard Alert!</strong> You have{" "}
          {subAlert.zombieCount} subscription
          {subAlert.zombieCount > 1 ? "s" : ""} you haven't used in 30+ days,
          wasting <strong>~{fmt(subAlert.zombieWaste)}/month</strong>.{" "}
          <a href="/subscriptions" style={{ color: "#fbbf24" }}>
            Review them →
          </a>
        </div>
      )}
      ...
      <div className="card-grid">
        {[
          {
            label: "Income",
            val: fmt(summary.income),
            sub: "this month",
            color: "#34d399",
          },
          {
            label: "Expenses",
            val: fmt(summary.expense),
            sub: `${budgetPct}% of budget`,
            color: budgetPct > 90 ? "#f87171" : "#e2e8f0",
          },
          {
            label: "Savings",
            val: fmt(summary.savings),
            sub: summary.savings >= 0 ? "Great job!" : "Over income",
            color: summary.savings >= 0 ? "#34d399" : "#f87171",
          },
          {
            label: "Budget left",
            val: fmt(Math.max(0, budget - summary.expense)),
            sub: `of ${fmt(budget)}`,
            color: budget - summary.expense < 0 ? "#f87171" : "#e2e8f0",
          },
        ].map((c) => (
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        >
          ...
          <span
            style={{
              fontSize: 13,
              color: budgetPct > 90 ? "#f87171" : "#94a3b8",
            }}
          >
            {budgetPct}% used
          </span>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="progress-track" style={{ height: 5 }}>
                  <div
                    className="progress-fill"
                    style={{
                      width: `${Math.round((c.value / (summary.expense || 1)) * 100)}%`,
                      background: CAT_COLORS[c.name] || "#64748b",
                    }}
                  />
                </div>
              </div>
            ...
            {insights.map((ins, i) => (
              <div
                ...
                style={{
                  ...
                  background: "#0f0f1a",
                  borderRadius: 8,
                  borderLeft: `3px solid ${insightColors[ins.type] || "#6366f1"}`,
                }}
              >
                <div style={{ flex: 1 }}>
                  <div
                    style={{
                      fontSize: 13,
                      fontWeight: 600,
                      color: insightColors[ins.type] || "#e2e8f0",
                      marginBottom: 3,
                    }}
                  >
                    {ins.title}
