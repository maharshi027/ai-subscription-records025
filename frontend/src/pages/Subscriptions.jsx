...
export default function Subscriptions() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        totalMonthly: res.data.totalMonthly,
        zombieCount: res.data.zombieCount,
        zombieWaste: res.data.zombieWaste,
      });
    } catch (err) {
      console.error(err);
    }
  ...
  const cancel = async (id, name) => {
    if (!window.confirm(`Cancel ${name}? This will mark it as inactive.`))
      return;
    try {
      ...
      <div className="card-grid" style={{ marginBottom: 20 }}>
        ...
        <div className="metric-card">
          ...
          <div
            ...
            style={{ color: stats.zombieCount > 0 ? "#f59e0b" : "#34d399" }}
          >
            {stats.zombieCount}
          </div>
        ...
        <div className="metric-card">
          ...
          <div
            className="metric-val"
            style={{ color: stats.zombieWaste > 0 ? "#f87171" : "#34d399" }}
          >
            {fmt(stats.zombieWaste)}
          </div>
      ...
      {zombies.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          ...
          >
            ...
            >
              🪦 Subscription Graveyard — {zombies.length} zombie
              {zombies.length > 1 ? "s" : ""} found
            </div>
            <div style={{ fontSize: 13, color: "#92400e", lineHeight: 1.6 }}>
              These subscriptions haven't been used in{" "}
              <strong style={{ color: "#fbbf24" }}>30+ days</strong> but are
              still charging you. You're wasting{" "}
              <span className="zombie-waste">
                {fmt(stats.zombieWaste)}/month
              </span>{" "}
              ={" "}
              <strong style={{ color: "#fb923c" }}>
                {fmt(stats.zombieWaste * 12)}/year
              </strong>{" "}
              on services you don't use. Cancel or mark as used.
            </div>
          </div>
            ...
            {zombies.map((s) => (
              <div key={s._id} className="sub-card zombie">
                ...
                >
                  <div>
                    ...
                    <span className="badge badge-warning">
                      🪦 Zombie — {s.daysSinceUse}d unused
                    </span>
                  </div>
                ...
                <div className="sub-renewal">
                  🔄 Renews in {s.daysUntilRenewal} days
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                  <button
                    className="btn btn-success btn-sm"
                    onClick={() => markUsed(s._id)}
                    style={{ flex: 1 }}
                  >
                    ✓ Mark used
                  </button>
          ...
          <div className="sub-grid">
            {active.map((s) => (
              <div key={s._id} className="sub-card">
                ...
                <div className="sub-renewal">
                  🔄 Renews in {s.daysUntilRenewal} days · Used {s.daysSinceUse}
                  d ago
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                  <button
      ...
      {subs.length === 0 && !loading && (
        ...
        >
          ...
          <div style={{ fontSize: 13 }}>
            Add your subscriptions to detect zombie ones wasting your money
          </div>
        </div>
      )}
